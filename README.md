...
ブラウザだけで動く、単一 HTML ファイルの Markdown プレビューアです。サーバーやビルドは不要で、`index.html` を開くだけで使えます。
...
1. 上記 URL を開くか、ローカルでは `index.html` をブラウザで開く
...
```
jet-md-viewer/
├── index.html   # アプリ本体（GitHub Pages）
└── README.md
```

## 技術メモ

- Markdown → HTML は `index.html` 内の `parseMarkdown()` で処理
